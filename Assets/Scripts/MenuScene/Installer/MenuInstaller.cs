...
namespace MenuScene.Installer
{
    public class MenuInstaller : MonoInstaller<MenuInstaller>
    {
        ...
        {
            // Container.BindInterfacesTo<NavigationPanelController>().AsSingle().WithArguments(_navigationView).NonLazy();
            //
            // Container.BindInterfacesTo<MenuPanelController>().AsSingle().WithArguments(_menuPanelsHandler).NonLazy();
